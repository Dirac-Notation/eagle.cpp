...
static std::map<llm_arch, const char *> LLM_ARCH_NAMES = {
    ...
    { LLM_ARCH_LLAMA_EAGLE,     "llama-eagle"},
...
struct llama_context {
    ...
    mutable struct ggml_tensor * last_feature;   // F32 [n_embd, n_batch]
...
static void llm_load_hparams(
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_LLAMA_EAGLE:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                model.type = e_model::MODEL_EAGLE; break;
            } break;
    ...
    // create tensors for the weights
    {
        ...
        switch (model.arch) {
            ...
            case LLM_ARCH_LLAMA_EAGLE:
                {
                    model.tok_embd = ml.create_tensor(ctx_input, tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});
                    model.fc = ml.create_tensor(ctx_input, tn(LLM_TENSOR_FC, "weight"), {2*n_embd, n_embd});
                    model.fc_b = ml.create_tensor(ctx_input, tn(LLM_TENSOR_FC, "bias"), {n_embd});
...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        size_t cur_size = ggml_type_size(cur->type);
        size_t view_offset = 4096*(cur->ne[1] - 1);
        lctx.last_feature = ggml_view_4d(ctx0, cur, 4096, 1, 1, 1, 4096*cur_size, cur_size, cur_size, view_offset);
    ...
    struct ggml_cgraph * build_llama_eagle() {
        struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, LLAMA_MAX_NODES, false);
        ...
        /////////////////////////////////////////////////////////////////
        //// 나중에 빼야 댐 ////
        lctx.last_feature = inpL;
        ///////////////////////

        struct ggml_tensor * cated_inp_feature = ggml_emb_feature_cat(ctx0, inpL, lctx.last_feature);
        cb(cated_inp_feature, "cated_inp_feature", -1);

        struct ggml_tensor * squeezed_inp_feature = ggml_mul_mat(ctx0, model.fc, cated_inp_feature);
        cb(squeezed_inp_feature, "squeezed_inp_feature", -1);

        squeezed_inp_feature = ggml_add(ctx0, squeezed_inp_feature, model.fc_b);
        cb(squeezed_inp_feature, "squeezed_inp_feature", -1);
        /////////////////////////////////////////////////////////////////
        ...
        for (int il = 0; il < n_layer; ++il) {
            struct ggml_tensor * inpSA = squeezed_inp_feature;
            
            // norm
            cur = llm_build_norm(ctx0, squeezed_inp_feature, hparams,
                    model.layers[il].attn_norm, NULL,
                    LLM_NORM_RMS, cb, il);
            cb(cur, "attn_norm", il);
            ...
            // input for next layer
            squeezed_inp_feature = cur;
        }

        cur = squeezed_inp_feature;
        cb(cur, "result_norm", -1);
...
static int llama_decode_internal(
    ...
    ggml_backend_sched_graph_compute(lctx.sched, gf); // 실제 연산 이루어지는 곳?
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        // graph inputs
        {
            ...
            ctx->last_feature = ggml_new_tensor_4d(ctx->ctx_input, GGML_TYPE_F32, 4096, 512, 1, 1);
            ...
            ggml_set_name(ctx->last_feature, "last_feature");
