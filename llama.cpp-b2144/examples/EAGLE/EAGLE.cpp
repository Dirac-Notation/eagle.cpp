...
int main(int argc, char ** argv) {
    ...
    // argv를 parse해서 params에 저장
    // mparams, cparams, sparams가 있고, 각각 모델 관련 context 관련 sampling 관련
    ...
    // max number of parallel drafting sequences (i.e. tree branches), 기본적으로 1
    ...
    log_set_target(log_filename_generator("EAGLE", "log"));
    LOG_TEE("Log start\n");
    ...
    while (true) {
        ...
        for (int i = 0; i < n_draft; ++i) {
            ...
            for (int s = 0; s < n_seq_dft; ++s) {
                ...
                for (int k = 0; k < std::min(n_seq_dft, (int) cur_p.size()); ++k) {
